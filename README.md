...
Crawfish Writer is a native macOS writing app focused on distraction-free drafting with plain text and Markdown.
...
```bash
swift build
swift run QuietWrite
```

You can also open `Package.swift` in Xcode and run the `QuietWrite` executable target.
